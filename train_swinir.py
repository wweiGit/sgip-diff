...
from sgip.utils.common import instantiate_from_config, calculate_psnr_pt, to
...
def main(args) -> None:
    ...
    if cfg.train.resume:
        pretrain = torch.load(cfg.train.resume, map_location="cpu")  
        ...
        swinir.load_state_dict(pretrain, strict=True)
        if accelerator.is_local_main_process:
            print(f"load SwinIR from {cfg.train.resume}")
    else:
        if accelerator.is_local_main_process:
    ...
    log_file =os.path.join(exp_dir, config_name + ".txt")
    ...
    while global_step < max_steps:
        ...
        for batch in loader:
            ...
            lq, gt, _, _ = batch["lq"], batch["gt"], batch["emoca"], batch["img_name"]
            ...
            # Save checkpoint:
            if global_step % cfg.train.ckpt_every == 0 or global_step == max_steps:
                if accelerator.is_local_main_process:
                    checkpoint = pure_swinir.state_dict()
                    ckpt_path = f"{ckpt_dir}/{global_step:07d}.pt"
                    torch.save(checkpoint, ckpt_path)

            if global_step % cfg.train.image_every == 0:
                swinir.eval()
                N = 12
            ...
            if global_step % cfg.train.val_every == 0 or global_step == max_steps:
                ...
                save_path_sub = os.path.join(exp_dir, "results", str(global_step))
                os.makedirs(save_path_sub, exist_ok=True)
                for j, val_batch in enumerate(val_loader):
                    to(val_batch, device)
                    val_batch = batch_transform(val_batch)
                    val_lq, val_gt, _, img_name = val_batch["lq"], val_batch["gt"], val_batch["emoca"], val_batch["img_name"]
                    val_gt = (
                    ...
                    ##########遍历 batch 内的每张图像并保存############
                    if j < 10:
                        for img_idx in range(val_pred.shape[0]):
                            save_path = os.path.join(save_path_sub, img_name[img_idx])
                            save_img_ori(val_pred[img_idx], save_path)
                ...
                with open(log_file, 'a') as f:  # 'a'模式表示附加写入，不会覆盖文件内容
                    f.write("Steps:{}, Epoch:{}, loss:{} psnr:{}, lpips:{} \n".format(global_step, epoch, avg_val_loss, avg_val_psnr, avg_val_lpips))
