...
def prompt_decoder(text, image_embeds, tokenizer, text_encoder, device):
    with torch.no_grad():
        ...
        token_embs = text_encoder(input_ids=input_ids.to(device), return_token_embs=True)
        
        #print(input_ids.shape)
        #print(arcface_token_id)
        #print(face_embs_padded.shape)
        
        token_embs[input_ids==arcface_token_id] = face_embs_padded
        encoder_hidden_states = text_encoder(input_ids=input_ids.to(device), input_token_embs=token_embs)[0]

    return encoder_hidden_states


def uncond_prompt_decoder(text, image_embeds, tokenizer, text_encoder, device):
    with torch.no_grad():
        ...
        encoder_hidden_states = text_encoder(input_ids=input_ids.to(device), return_token_embs=True)
