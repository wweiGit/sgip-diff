...
class SwinIR(nn.Module):
    ...
    def __init__(
        ...
        if unshuffle:
            ...
            #if fusion=="cat":
            #    self.conv_first = nn.Conv2d(num_in_ch * 2, embed_dim, 3, 1, 1)
            #else:
            #    self.conv_first = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1)
        
        # if self.shape_3d:
        #    self.conv_3dmm = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            # for real-world SR
            # if self.shape_3d and emoca.any():
            #    if self.fusion == "cat":
            #        x = torch.cat((x, emoca), dim=1)
            #        x = self.conv_first(x)

            #    else:        
            #        x = self.conv_first(x)
            #        emoca = self.conv_3dmm(emoca)
            #        x = x + emoca
